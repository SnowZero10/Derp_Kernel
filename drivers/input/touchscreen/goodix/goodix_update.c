...
extern void gt9xx_ts_register_productinfo(struct goodix_ts_data* ts_data);
extern void gtp_reset_guitar(struct i2c_client *client, s32 ms);
extern s32  gtp_send_cfg(struct i2c_client *client);
extern s32 gtp_read_version(struct goodix_ts_data *, u16* );
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
    ...
    u32 *cfg_info_len = ts->pdata->config_data_len;
#endif
    ...
    config[RESOLUTION_LOC]     = (u8)ts->pdata->x_max;
    config[RESOLUTION_LOC + 1] = (u8)(ts->pdata->x_max>>8);
    config[RESOLUTION_LOC + 2] = (u8)ts->pdata->y_max;
    config[RESOLUTION_LOC + 3] = (u8)(ts->pdata->y_max>>8);
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    GTP_DEBUG("Firmware length:%d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
...
s32 get_gup_update_file(void *dir)
{
    s32 ret = 0;
    st_fw_head fw_head;
    struct goodix_ts_data *ts = NULL;
    
    GTP_DEBUG("[get_gup_update_file]Begin update ......");
    ...
    if(FAIL == ret)
    {
        GTP_ERROR("[get_gup_update_file]check update file fail.");
        goto file_fail;
    }
    ...
    if(FAIL == ret)
    {
        GTP_ERROR("[get_gup_update_file]get ic message fail.");
        goto file_fail;
    }	
    ret = gup_enter_update_judge(&fw_head);
	ts->fw_version = fw_head.vid;
    if(FAIL == ret)
    {
        GTP_INFO("[get_gup_update_file]enter update judge fail.");
		ret = -2;
        goto file_fail;
    }
...
int factory_get_fs_fw_version(struct device *dev, char *buf)
{
	...
	get_fw_filename(fw_path,sizeof(fw_path));
	ret = get_gup_update_file(fw_path);
	if (ret == FAIL ){
		GTP_ERROR("[factory_get_fs_fw_version]Failed to get_gup_update_file");
		return -EIO;
	}
...
{	
#ifdef CONFIG_TOUCHSCREEN_GT9XX_AUTO_UPDATE
	s32 ret = 0;
	...
	get_fw_filename(fw_path,sizeof(fw_path));
	if(data->fw_error){
		printk("%s previous update is error,need update now.\n",__func__);
		return true;
	}
	ret = get_gup_update_file(fw_path);
	if (ret != SUCCESS){
		GTP_INFO("[factory_check_fw_update_need]No need to update bin file");
		need_update = false;
	}else{
		need_update = true;//need update .bin
	}
...
int factory_proc_fw_update(struct device *dev, bool force)
{
	...
	get_fw_filename(fw_path,sizeof(fw_path));
	if(data->gtp_is_suspend){
		data->fw_updating = 0;
		GTP_ERROR("[factory_proc_fw_update]IC enter suspend.");
		return -EPERM;
	}
	ret = gup_update_proc(fw_path);
	if(ret == FAIL){
		GTP_ERROR("[factory_proc_fw_update]Failed to get_gup_update_file");
	}
	gt9xx_ts_register_productinfo(data);
	data->fw_updating = 0;
	return 0;
}
...
int factory_proc_fw_bin_update(struct device * dev, const char *buf)
{
	...
	ret = gup_update_proc(fw_path);
	if(ret == FAIL){
		GTP_ERROR("[factory_proc_fw_bin_update]Failed to get_gup_update_file");
	}
	gt9xx_ts_register_productinfo(data);
	data->fw_updating = 0;
	return 0;
